...
from models import TradeEvent, Book, BookUpdate
...
@router.get("/", response_description="List all trade events", response_model=List[TradeEvent])
def list_trade_events(request: Request):
    books = list(request.app.database["ema_data"].find(limit=100))
    return books
